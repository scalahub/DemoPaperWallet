...
import org.sh.cryptonode.util.CommonUtil._
import org.sh.cryptonode.util.Json2XML._
import org.sh.cryptonode.util.StringUtil._
import org.sh.cryptonode.util.BytesUtil._
import org.sh.cryptonode.btc.TxParser
import org.sh.cryptonode.btc.BitcoinUtil
  ...
  def createRawTransaction(ins:Array[TxIn], outs:Array[TxOut]) = {
    ...
    val out = outs.map{
      case TxOut(Some(addr), amtBigInt) =>
        ...
        s"""\"$addr\":$amtBtc"""
      case TxOut(None, _) => throw new Exception("No address found in output")        
    }.reduceLeft(_+","+_)
